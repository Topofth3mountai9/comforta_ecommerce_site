...
export async function fetch_frequently_used_products({ url, filters }) {
  const { frequently_viewed_items_page, limit, sort_by } = filters;
  ...
  try {
    ...
    if (cached_data) {
      ...
      //applying pagination
      const start = (frequently_viewed_items_page - 1) * limit;
